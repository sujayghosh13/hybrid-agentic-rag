...
class Settings:
    ...
    rerank_candidates_count: int = int(os.getenv("RERANK_CANDIDATES_COUNT", "8"))
    ...
    ollama_think: bool = os.getenv("OLLAMA_THINK", "false").lower() in ("true", "1", "yes")
    ...
    query_rewriter_enabled: bool = os.getenv("QUERY_REWRITER_ENABLED", "false").lower() in ("true", "1", "yes")
    conversational_memory_enabled: bool = os.getenv("CONVERSATIONAL_MEMORY_ENABLED", "true").lower() in ("true", "1", "yes")
    conversational_memory_turns: int = int(os.getenv("CONVERSATIONAL_MEMORY_TURNS", "2"))
    ...
    crag_high_confidence_score: float = float(os.getenv("CRAG_HIGH_CONFIDENCE_SCORE", "2.0"))
